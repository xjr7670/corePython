...
def connect(db, dbName):
    ...
    if db == 'sqlite':
        ...
        try:
            import MySQLdb
            import _mysql_exception as DB_DEC
        except ImportError, e:
            return None

        try:
            cxn = MySQLdb.connect(db=dbName)
        except _mysql_executions.OperationalError, e:
            cxn = MySQLdb.connect(user='root', passwd='root', db=dbName)
        try:
...
def main():
    ...
    print 'from one groud (%d) to another (%d)' % (fr, to)
